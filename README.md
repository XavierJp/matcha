...
Pour déployer et faire fonctionner l'application sur un server dédié dans Azure il faut récupérer le contenu du répository matcha-infra.

Le répository matcha-infra est lui, privé car il est utilisé pour contenir l'ensemble des données sensibles (Clés SSH, mots de passes ...) nécessaires à la mise en place de votre application, merci de suivre sa documentation dédiée pour déployer l'application.
...
- Un réseau _défini dans `matcha_network` du docker-compose_.
...
```bash
docker exec -t -i matcha_server /bin/bash
```

De même pour consulter la liste des fichiers dans le docker :

```bash
docker exec matcha_server bash -c 'ls'
```
